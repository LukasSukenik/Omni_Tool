...
public:
    ...
    void generate( Data& data )
    {
        ...
        double outer_leaflet_radius = data.in.p_float["Radius"]+4.0;
        double inner_leaflet_area = pow(data.in.p_float["Radius"], 2);
        ...
        int lower_leaflet_lipid_count = data.in.p_int["Num_lipids"] * 1.0 / (1.0 + sphere_surface_relative_increase);
        int lower_leaflet_receptor_count = data.in.p_int["Number_of_receptors"] * 1.0/ (1.0 + sphere_surface_relative_increase);
...
private:
    ...
    {
        if(data.in.p_int["Number_of_receptors"] > data.in.p_int["Num_lipids"])
        {
            cerr << "Number of receptors " << data.in.p_int["Number_of_receptors"] << " cannot be larger than number of lipids " << data.in.p_int["Num_lipids"] << endl;
            exit(1);
        }
    }
