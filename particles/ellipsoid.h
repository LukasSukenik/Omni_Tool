...
private:
    ...
    void generate_const_surf_distrib()
    {
        ...
        double cumul_surf = 0.0;
        double cumul_surf_prev = 0.0;
        double total_surf = 0.0;

        double z_increment = c/real_samples;
        double j_z;
        ...
        double surf_increment = total_surf / (samples-1);
        ...
        for ( int j = -real_samples+2; j <= real_samples; j+=2 )
        {
            ...
            if(cumul_surf > cumul_surf_prev + surf_increment && const_surf_Z_distrib.size() < samples)
            {
                ...
                //cout << ":" << j_z << " diff " << j_z-const_surf_Z_distrib.back() << endl;
    ...
    Atom get_atom(double deg, double z)
    {
        ...
        add.pos.x = a * sqrt( 1 - ((z*z) / (c*c)) ) * cos (deg*deg_to_rad);
        add.pos.y = b * sqrt( 1 - ((z*z) / (c*c)) ) * sin (deg*deg_to_rad);
        add.pos.z = z;
...
public:   
    ...
    void generate( Data& data )
    {
        ...
        // Validation
        //
        /*if( validate(beads, 1.0, 1.0, data.in.c) )
...
protected:
    ...
    void fibonacci_tri_axial_spheroid(vector<Atom>& container, int samples, double b, double c, const int type, int orientation)
    {
    ...
    bool validate(vector<Atom>& container, double a, double b, double c)
    {
        ...
        {
            value = (item.pos.x*item.pos.x) / (a*a);
            value += (item.pos.y*item.pos.y) / (b*b);
            value += (item.pos.z*item.pos.z) / (c*c);
    ...
    void distribution(vector<Atom>& container, double a, double b, double c)
    {
        ...
        for(auto& item : container)
        {
            ++x[ (int) (  (item.pos.x+a)*(size/(2*a))  ) ];
            ++y[ (int) (  (item.pos.y+b)*(size/(2*b))  ) ];
            ++z[ (int) (  (item.pos.z+c)*(size/(2*c))  ) ];
...
private:
    ...
    double get_max_x()
    {
        double max = beads[0].pos.x;
        for(int i=0; i<beads.size(); ++i)
        {
            if(beads[i].pos.x > max)
                max = beads[i].pos.x;
        }
        return max;
    }
    ...
    double get_max_y()
    {
        double max = beads[0].pos.y;
        for(int i=0; i<beads.size(); ++i)
        {
            if(beads[i].pos.y > max)
                max = beads[i].pos.y;
        }
        return max;
    }
    ...
    double get_max_z()
    {
        double max = beads[0].pos.z;
        for(int i=0; i<beads.size(); ++i)
        {
            if(beads[i].pos.z > max)
                max = beads[i].pos.z;
        }
        return max;
    }
    ...
    double get_min_x()
    {
        double min = beads[0].pos.x;
        for(int i=0; i<beads.size(); ++i)
        {
            if(beads[i].pos.x < min)
                min = beads[i].pos.x;
        }
        return min;
    }
    ...
    double get_min_y()
    {
        double min = beads[0].pos.y;
        for(int i=0; i<beads.size(); ++i)
        {
            if(beads[i].pos.y < min)
                min = beads[i].pos.y;
        }
        return min;
    }
    ...
    double get_mim_z()
    {
        double min = beads[0].pos.z;
        for(int i=0; i<beads.size(); ++i)
        {
            if(beads[i].pos.z < min)
                min = beads[i].pos.z;
        }
        return min;
    }
